...
import { login, logout, refresh, register, weappLogin } from '@/api/auth'
import { getCurrentUser, updateUser, getPerms, getUserInfo } from '@/api/user'
import * as auth from '@/utils/auth'
import isEmpty from 'lodash/isEmpty'
import { updateUnionid } from '../../api/auth'
...
const actions = {
  ...
  async weappLogin ({ dispatch, commit }, params = {}) {
    const loginData = await wx.login()
    params.code = loginData.code

    const authResponse = await weappLogin(params)

    commit('setToken', authResponse.data)
    auth.setToken(authResponse.data)

    dispatch('getUser')
    if (!authResponse.data.unionid) {
      // 未绑定unionid, 则发送请求
      let params2 = await wx.getUserInfo()
      await updateUnionid(params2)
    }
  },
